...
\geometry{a6paper,landscape,hmargin={1cm,1cm},vmargin={0.5cm,0.2cm}}
...
\end{tikzpicture}%
% replace your current \obeylines lines with this scoped version
\obeylines {\addfontfeatures{Scale=3,LetterSpace=10} PARTY TIME}
...
% put this just before \newpage so it only appears on page 1

%%\includegraphics[width=1cm]{qr.png}
\smallskip
...
\node[anchor=south east,inner sep=2mm,fill=white,rounded corners=1mm,xshift=-5mm,yshift=5mm] at (current page.south east) {\includegraphics[width=15mm]{qr.png}};
\end{tikzpicture}
...
Pending the weather--which one month out looks bleak--we will (optimistically) have \mbox{table tennis} going, music, a fire, the oven and a cake at Midnight. Crashes are first come first served. \mbox{\color{MidnightBlue}Plus 1's welcome.}
